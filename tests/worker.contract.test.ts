...
class FakeWorker implements Worker {
  ...
  addEventListener(type: string, listener: EventListenerOrEventListenerObject) {
    ...
    arr.push(listener as (ev: any) => void)
  }

  removeEventListener(type: string, listener: EventListenerOrEventListenerObject) {
    const arr = this.listeners.get(type)
    if (!arr) return
    const idx = arr.indexOf(listener as (ev: any) => void)
  ...
  dispatch(type: string, data: any) {
    ...
    if (type === 'message' && this.onmessage) this.onmessage.call(this, ev as unknown as MessageEvent)
  }

  postMessage(message?: any) {
    const cmd = message as { type: string; payload: any; id: string; timestamp: number }
    switch (cmd.type) {
      case 'INIT': {
        const p = cmd.payload || { timestep: 0.1, bufferSize: 10000, debugMode: false }
      ...
      case 'RESET': {
        ...
        this.t = 0
        this.PV = 25
        const preserve = !!(cmd as any).payload?.preserveParams
      ...
      case 'SET_SP': {
        this.SP = (cmd as any).payload?.value ?? this.SP
        break
      }
      case 'SET_PID': {
        // ignorado en fake
